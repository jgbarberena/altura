...
let servicioEditandoId = null // id de availability si estamos editando
let hitosProvTemp      = []
...
inputProveedorId.addEventListener('input', () => {
    ...
    if (coincidencias.length > 0) {
        ...
        autoProvList.style.display = 'block'
    } else {
        autoProvList.style.display = 'none'
    }
...
// Guardar cambios automáticamente en proveedor existente
...
inputPlazas.addEventListener('input', actualizarBtnServicio)
inputPrecio.addEventListener('input', actualizarBtnServicio)
...
function limpiarFormularioServicio() {
    ...
    hitosProvTemp = []
    document.getElementById('panel-pagos-proveedor').style.display   = 'none'
    document.getElementById('contenido-pagos-proveedor').style.display = 'none'
    document.getElementById('togglePagosProveedorIcon').textContent   = '▶'
...
// Panel de pagos proveedor — toggle
document.getElementById('togglePagosProveedor').addEventListener('click', () => {
    const c    = document.getElementById('contenido-pagos-proveedor')
    const icon = document.getElementById('togglePagosProveedorIcon')
...
async function persistirHitosProveedor(proveedorId) {
    // Borrar pagos existentes y reinsertar los del array temporal
...
async function cargarHitosPagoProveedor(proveedorId) {
    ...
    renderHitosProveedor()
    document.getElementById('panel-pagos-proveedor').style.display = 'block'
}
...
btnGuardarServicio.addEventListener('click', async () => {
    ...
    if (servicioEditandoId) {
        // MODO EDITAR: actualizar availability
        ...
        // MODO CREAR: insertar en availability
    ...
    // Persistir hitos de pago
    await persistirHitosProveedor(proveedorActual.id)

    // Refrescar datos
    todaDisponibilidad = (await supabase.from('availability').select('*')).data
...
async function cargarServiciosProveedor(proveedorId) {
    ...
    const payments  = todosPayments.filter(p => p.provider_id === proveedorId)
    ...
    const pendiente = payments.filter(p => !p.paid).reduce((s, p) => s + parseFloat(p.amount), 0)

    bloque.style.display = 'block'
    tbody.innerHTML = dispProv.map(d => `
        <tr data-disp-id="${d.id}">
            ...
            <td>${d.billing_model === 'consumption'
                ...
                : 'Capacidad'}</td>
            <td class="ok">${fmt(pagado)}</td>
            <td class="warn">${fmt(pendiente)}</td>
        </tr>
    `).join('')

    // Botón editar
    document.querySelectorAll('#tbody-servicios-proveedor tr').forEach(tr => {
        tr.addEventListener('click', e => {
            ...
            cargarHitosPagoProveedor(proveedorActual.id)
...
async function eliminarServicioSeleccionado() {
    ...
    const servicioId  = disp.service_id
    const proveedorId = disp.provider_id

    // ¿Hay reservas con este proveedor+servicio?
    ...
    // ¿Algún otro proveedor ofrece este servicio?
    const otrosProveedores = todaDisponibilidad.filter(d =>
        d.service_id === servicioId && d.provider_id !== proveedorId
    )

    // ¿Es el último servicio de este proveedor?
    const serviciosDelProv = todaDisponibilidad.filter(d => d.provider_id === proveedorId)
    const esUltimoServicio = serviciosDelProv.length === 1

    let borrarServicio  = false
    let borrarProveedor = false

    if (otrosProveedores.length === 0) {
        // Nadie más ofrece este servicio
        const opcion = confirm(
            `El servicio "${servicioId}" no lo ofrece ningún otro proveedor.\n\n` +
            `¿Deseas eliminar también el servicio de la tabla de servicios?\n\n` +
            `Aceptar = Eliminar disponibilidad Y servicio\n` +
            `Cancelar = Eliminar solo la disponibilidad de este proveedor`
        )
        borrarServicio = opcion
    }

    if (esUltimoServicio) {
        const opcion = confirm(
            `Este es el último servicio de "${proveedorId}".\n\n` +
            `¿Deseas eliminar también el proveedor?\n\n` +
            `Aceptar = Eliminar disponibilidad Y proveedor\n` +
            `Cancelar = Eliminar solo la disponibilidad`
        )
        borrarProveedor = opcion
    }
...
// Botón eliminar — lo añadimos al bloque 3
document.getElementById('bloque-servicios-proveedor').addEventListener('click', e => {
    if (e.target.id === 'btnEliminarServicio') eliminarServicioSeleccionado()
})